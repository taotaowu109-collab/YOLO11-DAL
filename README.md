...
Private working repository for a **paper-faithful reconstruction** of:
...
>
> This repository is reconstructed from the current manuscript and the cited source designs. It is **not presented as recovered original author source code**. Exact equality with the manuscript's reported parameter counts, GFLOPs and detection metrics must be verified against the original training artifacts before a public release or DOI archive is described as the paper's underlying code.

## Implemented

- YOLO11n baseline.
- P2-P5 control model for the reviewer-requested `YOLO11n + P2 + original head` experiment.
- C3k2-LEGM reconstruction.
...
- Reference CSV containing the manuscript and response-letter target values.

See [`docs/MAPPING_TO_MANUSCRIPT.md`](docs/MAPPING_TO_MANUSCRIPT.md) for the exact manuscript-to-code mapping and reconstruction assumptions.
...
This reconstruction is pinned to `ultralytics==8.4.157` because its internal API is used by the variant builder.
...
The exact original augmentation settings, seed and DCNv3 source implementation are not completely specified in the manuscript. These must be verified before an exact-reproduction claim.
...
The values below are copied from the manuscript/response letter into `reference/reported_metrics.csv`. They are **reference targets, not results generated by this reconstruction**.
...
## DCNv3 reconstruction note

The manuscript specifies DCNv3, but the current manuscript files do not contain the exact original operator source/configuration. The repository therefore includes `DCNv3Lite`, implemented with `torchvision.ops.deform_conv2d` and learned offsets/masks.

This is a transparent placeholder for reproducibility work, **not a claim of exact equivalence to the original unpublished DCNv3 code**. If the original operator/config is available later, it should replace this adapter and all ablations should be rerun.
...
4. Resolve every reconstruction choice in `docs/MAPPING_TO_MANUSCRIPT.md`.
...
7. Only then describe that release as code supporting the published results.
